...
def create_CC_model(kmer_len,filter_num,units):
    ...
    #Conv1d layer1
    model.add(layers.Conv1D(filters = filter_num,kernel_size = 12,strides = 1,padding = "valid",input_shape = (9000-kmer_len,100),activation = "relu"))
...
def padding_truncate_seq(seq):
    ...
    if length > 9000: 
        ...
        seq =  seq + (9000 -length)*'N'  #padding N after seq to 4000nt
    return seq
...
# Get input_file path
if isnotebook():
    training_f = "/data/rnomics8/yuanguohua/RNA-Light_Original/mRNA/03_Model_Construction/mRNA_sublocation_TrainingSet.tsv"
    test_f = "/data/rnomics8/yuanguohua/RNA-Light_Original/mRNA/03_Model_Construction/mRNA_sublocation_TestSet.tsv"
else:
    args = get_filepath()
...
# Load word2vec model
word2vec_model = Word2Vec.load("/data/rnomics8/yuanguohua/RNAlight_review/mRNA/word2vec/Pretrained_CNN_word2vec_model/00_pretrained_CNN_word2vec_" + str(kmer_len) + "_mer_len.model")
